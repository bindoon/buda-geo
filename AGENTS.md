...
主方案见 `docs/布达-GEO工具详细解决方案.md`。此处只写**读代码也容易做错**的约定。
...
- ✅ 密码进根 `.env`、`projects/{名}/.secrets.env`（均已 gitignore）或环境变量；仓库只提交 `.env.example` 和环境变量名。
...
- ❌ 开源仓库中不要提交演示账号口令；README、docs、Issue、日志、probe、文章和回执都不能出现真实凭证。
...
- ✅ 把对标结论抽象成布达自己的通用事实、诊断、场景、内容和发布方法；研发说明写在 `docs/`、OpenSpec 或本文件。
- 最终运行交付面只有 `packages/geo-cli`、`skills/buda-skills` 和 `projects/{项目名}/` 目录契约；`docs/`、`openspec/`、竞品资料不应成为运行时依赖。
...
- 发布必须从 `approvedArticleInput` 经 `publish prepare → authorize → record → validate`；❌ 不要跳过 dry-run/授权，也不要把 `submitted` 解释为 `published`。
- `publish/destinations.json` 只写已评级目标和环境变量名；`adapter` 只是契约预留，具体适配器未实现/审核时不得执行或声称自动发布。
...
- 授权发布：`openspec/changes/add-authorized-publishing-workflow/`（已完成待归档）。
