...
- 用户说“审稿、批准” → 阶段 6。
- 用户说“发布、投放” → 阶段 7；外部写入必须有明确授权。
- 用户说“复测、周报、优化下一轮” → 阶段 8。
- 用户说“同步上云、客户门户” → 阶段 9；默认只同步展示数据，不在 SaaS 重算本地能力。
...
- `public` 事实才可进入公开内容；restricted/internal 资料不用于文章、发布或 SaaS 对客展示。
- 无客服/询盘记录是建议项，不阻断企业事实或场景确认。
- clean 不创建或刷新关键词、FAQ、Prompt、generation plan、诊断题、文章或配额。
- 每一阶段消费上一阶段的**已确认版本**，新变化创建新版本，不覆盖历史确认快照。
- 内容规划按 Topic bundle 给普通用户复核，报告必须列出实际事实内容；只有 confirmed content plan 的 approved ready/research-only tasks 能进入文章生成。
- 文章由 CLI 写作包约束、本地 Agent 语义生成、CLI ingest；生成结果永远是 `draft + requires_human_review`，必须进入独立审稿阶段。
