...
export interface ArticleWritingBrief {
  ...
  audience: string;
  questions: string[];
  desired_next_action: Record<string, unknown> | null;
  outline_requirements: string[];
  tone: string;
  channel_constraints: string[];
  citation_policy: string;
  allowed_facts: ArticleFactView[];
  allowed_images: ArticleImageView[];
...
export interface ArticleReviewAssessment {
  schema_version: 1;
  article_id: string;
  body_sha256: string;
  checks: {
    factual_accuracy: ArticleReviewCheck;
    claim_boundaries: ArticleReviewCheck;
    channel_fit: ArticleReviewCheck;
    compliance: ArticleReviewCheck;
    originality: ArticleReviewCheck;
  };
  summary: string;
}
export interface ArticleReviewRecord {
  ...
  reason: string;
  assessment: ArticleReviewAssessment;
  body_sha256: string;
  at: string;
}
export interface ArticleMeta {
  ...
  risks: ArticleRisk[];
  review_history: ArticleReviewRecord[];
  created_at: string;
  updated_at: string;
}
